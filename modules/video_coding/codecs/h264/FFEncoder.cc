...
bool FFEncoder::init(ID3D11Device* d3d11device, int width, int height) {
  int ret;
  ...
  av_context_->time_base = AVRational { 1, 60 };
  av_context_->framerate = AVRational { 60, 1 };
  ...
  //Haichao:slice? It seems we do not slice only on old intel GPU.
  ...
  auto bitrate = 20000;//codec_settings->maxBitrate * 1000;
  av_context_->rc_max_rate    = bitrate;
  av_context_->rc_buffer_size = bitrate / 60;//maxframerate;
...
bool FFEncoder::init(const std::string& codec_name, const VideoCodec* codec_settings, ID3D11Device* d3d11device) {
    int ret;
