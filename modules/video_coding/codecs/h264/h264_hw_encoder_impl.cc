...
#include "modules/video_coding/codecs/h264/h264_hw_encoder_impl.h"
...
const bool kOpenH264HWEncoderDetailedLogging = false;
...
enum H264HWEncoderImplEvent {
  kH264HWEncoderEventInit = 0,
  kH264HWEncoderEventError = 1,
  kH264HWEncoderEventMax = 16,
};
...
// Helper method used by H264HWEncoderImpl::Encode.
...
static bool H264HWEncoderImpl::IsSupported(){
  //Todo:verify if it is correct here.
...
H264HWEncoderImpl::H264HWEncoderImpl(const cricket::VideoCodec& codec)
...
H264HWEncoderImpl::~H264HWEncoderImpl() {
  Release();
}

void do_nothing(void *) {}

int32_t H264HWEncoderImpl::InitEncode(const VideoCodec* inst,
  ...
  //TODO:(haichao) support simulcast?
  if (doing_simulcast) {
    ReportError();
    return WEBRTC_VIDEO_CODEC_ERR_PARAMETER;
  }
  ...
  for (int i = 0, idx = number_of_streams - 1; i < number_of_streams;
    ...
    if (hardware){
      ...
      AVBufferRef * m_HwFramesContext = av_hwframe_ctx_alloc(m_HwDeviceContext.get());
      ...
      if(auto err = av_hwframe_ctx_init(m_HwFramesContext); err < 0) {
        return err;
      }
      av_context_->hw_frames_ctx = av_buffer_ref(m_HwFramesContext);
    ...
    // we are going to set the d3d11 device. We will recive it in the frame to decode,
    // so set it during the first time we receive an image to encode.
    /* Haichao: don't forget to close and free everything at the end */
    //avcodec_free_context(&c);

    // Initialize encoded image. Default buffer size: size of unencoded data.
    
    // This assumes image in the cpu memory. We will move it to GPU in the future.
    ...
    RTC_DCHECK(openh264_encoder);
    if (kOpenH264HWEncoderDetailedLogging) {
...
int32_t H264HWEncoderImpl::Release() {
  ...
  //TODO(haichao): check instances to release.
...
int32_t H264HWEncoderImpl::RegisterEncodeCompleteCallback(
    EncodedImageCallback* callback) {
  encoded_image_callback_ = callback;
  return WEBRTC_VIDEO_CODEC_OK;
}

void H264HWEncoderImpl::SetRates(const RateControlParameters& parameters) {
  //TODO(haichao): update rate.
  if (encoders_.empty()) {
...
//prototype
/*
int32_t H264HWEncoderImpl::Encode(
    const VideoFrame& input_frame,
    const std::vector<VideoFrameType>* frame_types) {
    D3D11Texture2D* buffer = input_frame.video_frame_buffer()->GetNative();
    if (buffer == nullptr){
      ReportError();
      return WEBRTC_VIDEO_CODEC_ENCODER_FAILURE;
    }
    if (d3dDevice_ == nullptr){
      //ref d3dDevice frome frame
      // we need to add some settings when creating the device.
    }
...
int32_t H264HWEncoderImpl::Encode(
  ...
  // Haichao: How to bind this texture to avframe?
  ID3D11Texture2D* frame_buffer = (ID3D11Texture2D)input_frame.video_frame_buffer()->GetNative();
  if (!frame_buffer) {
    RTC_LOG(LS_ERROR) << "Failed to get D3D11Texture from Frame buffer. Can't encode frame.";
    return WEBRTC_VIDEO_CODEC_ENCODER_FAILURE;
  }
  ...
  /*Haichao: what it is used for? does it make windows sluttering?
  ...
  */
  ...
  auto ret = avcodec_send_frame(av_context_.get(), av_frame_.get());
  if(ret < 0) {
    RTC_LOG(LS_ERROR) << "Could not send a frame for encoding";
    return WEBRTC_VIDEO_CODEC_ENCODER_FAILURE;
  }

  while(ret >= 0) {
    auto packet = std::make_unique<AVPacket>(nullptr);

    ret = avcodec_receive_packet(av_context_.get(), packet.get());
    ...
    // how to tranfer packet to encoded_images_?
    
    /* haichao: what is it used for?
    if(session.inject) {
      if(session.inject == 1) {
...
// in InitializeExt.
SEncParamExt H264HWEncoderImpl::CreateEncoderParams(size_t i) const {
...
void H264HWEncoderImpl::ReportInit() {
  if (has_reported_init_)
    return;
  RTC_HISTOGRAM_ENUMERATION("WebRTC.Video.H264HWEncoderImpl.Event",
                            kH264HWEncoderEventInit, kH264HWEncoderEventMax);
  has_reported_init_ = true;
}

void H264HWEncoderImpl::ReportError() {
  if (has_reported_error_)
    return;
  RTC_HISTOGRAM_ENUMERATION("WebRTC.Video.H264HWEncoderImpl.Event",
                            kH264HWEncoderEventError, kH264HWEncoderEventMax);
  has_reported_error_ = true;
}

VideoEncoder::EncoderInfo H264HWEncoderImpl::GetEncoderInfo() const {
  EncoderInfo info;
  info.supports_native_handle = true;
  info.implementation_name = "H264-HardWare";
...
void H264HWEncoderImpl::LayerConfig::SetStreamState(bool send_stream) {
